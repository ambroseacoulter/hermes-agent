...
| `/autonomy [status\|pause\|resume\|inbox\|watch\|drafts\|runs]` | Inspect or control the profile-scoped gateway autonomy loop. |
...
- `/status`, `/sethome`, `/autonomy`, `/update`, `/approve`, `/deny`, and `/commands` are **messaging-only** commands.
