...
def _make_update_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "checkout" in joined and "main" in joined:
            return SimpleNamespace(stdout="", stderr="", returncode=0)
        if "checkout" in joined and current_branch in joined:
            return SimpleNamespace(stdout="", stderr="", returncode=0)
...
def test_cmd_update_stays_on_feature_branch_when_remote_branch_exists(monkeypatch, tmp_path):
    """When the current branch exists on origin, update stays on it."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
...
def test_cmd_update_switches_to_main_when_feature_branch_not_on_remote(monkeypatch, tmp_path, capsys):
    """When the current branch is local-only, update falls back to main."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    assert "falling back to main" in out
...
def test_cmd_update_restores_stash_when_already_up_to_date_on_remote_branch(monkeypatch, tmp_path, capsys):
    """When on a tracked feature branch with no updates, stash is restored in place."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
